...
function dfyt_get_default_options() {
	return {
		active: true,
...
function load_options(callback) {
	...
	chrome.storage.sync.get(['dfYoutubeOptions'], function(data) {
		...
		{
			options = dfyt_get_default_options();

			save_options();
		}
		else
		{
			options = align_objects(dfyt_get_default_options(), data.dfYoutubeOptions);
		}
...
function broadcast_DFYT_options(tabID) {
	...
	{
		chrome.tabs.query({}, function(tabs) { // TODO NEED TO ONLY CALL UPDATE	ON YOUTUBE TABS, NOT ALL
...
function save_options() {
	// chrome.storage.sync.set({dfYoutubeOptions: options}); // TODO remove this later... get extension working first...
}

function set_icon(icon) { // TODO remove this later... get extension working first...
	// if (icon == 'active')
	// {
